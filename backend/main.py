...
def create_timesheet(timesheet: TimesheetIn, db: Session = Depends(get_db)):
    ...
    for entry in timesheet.entries:
        ...
        if entry.from_time > entry.to_time:
            # Split overnight into two parts
            first_hours = ((datetime.combine(datetime.today(), time(23, 59)) -
                            datetime.combine(datetime.today(), entry.from_time)).seconds - (entry.break_minutes * 60)) / 3600.0
            ...
            second_hours = ((datetime.combine(datetime.today(), entry.to_time) -
                             datetime.combine(datetime.today(), time(0, 0))).seconds) / 3600.0
            ...
            total_hours = ((datetime.combine(datetime.today(), entry.to_time) -
                            datetime.combine(datetime.today(), entry.from_time)).seconds - (entry.break_minutes * 60)) / 3600.0
            ...
            entries_to_add.append((ts.date, entry.employee_emp_no, total_hours, normal_entry))

    for entry_date, emp_no, hours, entry_obj in entries_to_add:
        # Query total hours already logged for this employee on this date
...
def get_timesheets_by_ids(timesheet_ids: List[int] = Query(...), db: Session = Depends(get_db)):
    ...
    for timesheet in timesheets:
        ...
        timesheet.entries = [entry._asdict() for entry in entries]
